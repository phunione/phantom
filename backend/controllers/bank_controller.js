...
export const addBank = async (req, res) => {
    try {
      ...
      if (existingBank) {
        return res.status(409).json({ error: "Bank with the same unique_bank_id already exists" });
      }
...
const add_banker_id = async(req,res)=>{
  ...
  try {
    ...
    if (!existingBankerId) {
      return res.status(404).json({ success: false, message: 'Banker not found' });
    }
    ...
    return res.status(200).json({ success: true, message: 'banker id added to banker_ids', bankerId: existingBankerId });
  } catch (error) {
    console.log(error)
